...
const ProductCard = ({ product }) => {
  ...
  return (
    <Link to={`/product/${product.id}`} className="bg-white border hover:border-accent border-border flex flex-col h-full overflow-hidden group transition-all duration-300 hover:shadow-[0_8px_30px_rgba(236,72,153,0.12)]" id={`product-card-${product.id}`}>
      {/* Image section */}
      <div className="relative w-full overflow-hidden aspect-square bg-white border-b border-border p-2 shrink-0">
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="p-4 flex flex-col grow">
        ...
        {/* CTA */}
        <div className="mt-auto pt-3 border-t border-border flex items-center justify-end min-h-11 sm:min-h-0">
